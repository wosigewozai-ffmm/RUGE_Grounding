...
public class GroundAllRules {
	...
	public HashMap<String, Integer> MapEntity2ID = null;
	...
	public void readData(String fnEntityIDMap, String fnRelationIDMap,
		...
		while ((line = reader.readLine()) != null) {
			...
			MapEntity2ID.put(strRelation, iRelationID);
		}
		reader.close();
		...
		while((line = reader.readLine()) != null) {
			String[] tokens = line.split("\t");
			Integer iRelationID = MapRelation2ID.get(tokens[1]);
			String strValue = tokens[0] + "#" + tokens[2];
		...
		while((line = reader.readLine()) != null) {
			String[] tokens = line.split("\t");
			Integer iRelationID = MapRelation2ID.get(tokens[1]);
			Integer iSubjectID = MapEntity2ID.get(tokens[0]);
			Integer iObjectID = MapEntity2ID.get(tokens[2]);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (bodys.length == 3){
        			...
        			for (int iIndex = 0; iIndex < iSize; iIndex++) {
        				...
        				int iSubjectID = MapEntity2ID.get(strValue.split("#")[0]);
        				int iObjectID = MapEntity2ID.get(strValue.split("#")[1]);
	...
	public static void main(String[] args) throws Exception {
		...
		String fnRelationIDMap = "datasets\\fb15k\\relations.dict";
		String fnEntityIDMap = "datasets\\fb15k\\entities.dict";
        ...
        generator.PropositionalizeRule(fnEntityIDMap, fnRelationIDMap, fnRuleType, fnTrainingTriples, fnOutput);
